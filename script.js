...
[FUTURE_DIRECTIONS]`,
  ...
  const PARAMETER_DESCRIPTIONS = {
    ...
    "non-conceptual": {
      ...
      description:
        "Badanie bezpośredniego doświadczenia przed konceptualizacją",
    },
    presence: {
  ...
  const MODULE_INFO = {
    "I.1": {
      ...
      description:
        "Określa filozoficzne podstawy i kluczowe założenia procesu destylacji.",
      related: ["I.2", "I.3"],
    },
    "I.2": {
      name: "Ontologia Parametrów Destylacyjnych",
      description:
        "Definiuje naturę i strukturę parametrów używanych w procesie destylacyjnym.",
      related: ["I.1", "II.1"],
    },
    "I.3": {
      name: "Architektura Procesu Metadestylacyjnego",
      description:
        "Określa strukturę i relacje między komponentami systemu metadestylacyjnego.",
      related: ["I.1", "II.1", "II.2"],
    },
    "II.1": {
      name: "Protokół Destylacji Pierwszego Rzędu",
      description:
        "Procedura systematycznej analizy zjawiska przez pryzmat wybranych parametrów.",
      related: ["I.2", "II.2", "II.3"],
    },
    "II.2": {
      name: "Protokół Metadestylacji",
      description:
        "Procedura integracji wglądów z destylacji pierwszego rzędu.",
      related: ["II.1", "II.3", "II.4"],
    },
    "II.3": {
      name: "Protokół Wypalarki Metadestylacyjnej",
      description:
        "Procedura analizy tego, co zostało odfiltrowane w procesie destylacji.",
      related: ["II.1", "II.2"],
    },
    "II.4": {
      name: "Protokół Aplikacji Praktycznej",
      description:
        "Procedura przekładu wglądów destylacyjnych na konkretne działania.",
      related: ["II.2"],
    },
    "VI.1": {
      name: "Destylacja Wielowymiarowa",
      description:
        "Integracja różnych sposobów poznania (konceptualnego, ucieleśnionego, kontemplacyjnego i intersubiektywnego).",
      related: ["I.1", "II.1"],
    },
    "VI.2": {
      name: "Meta-Ramowanie",
      description:
        "Świadomość i rekonfiguracja założeń epistemologicznych procesu destylacyjnego.",
      related: ["I.1", "VI.1", "VI.3"],
    },
    "VI.3": {
      name: "Integracja Transkonceptualna",
      description:
        "Przekraczanie dualizmu konceptualnego i łączenie różnych wymiarów poznania.",
  ...
  const elements = {
    ...
    customTemplateContainer: document.getElementById(
  ...
  function bindEvents() {
    ...
    elements.projectNameInput.addEventListener(
      "input",
      (e) => (state.projectName = e.target.value)
    )
    elements.subjectDescriptionInput.addEventListener(
      "input",
      (e) => (state.subjectDescription = e.target.value)
    )
    ...
    elements.customTemplateInput.addEventListener(
      "input",
      (e) => (state.customTemplate = e.target.value)
    )
    ...
    elements.zoomInBtn.addEventListener("click", () => {
      scale *= 1.2
      elements.diagramGroup.setAttribute(
        "transform",
        `translate(40, 40) scale(${scale})`
      )
    })
  ...
  function toggleModuleInfo(moduleId) {
    ...
    showModal(
      ...
      `<p>${module.description}</p>
          ...
          <ul>${module.related
            .map((id) => `<li>${id}: ${MODULE_INFO[id]?.name || ""}</li>`)
            .join("")}</ul>
    ...
    document
      .getElementById("add-to-sequence-btn")
    ...
    state.sequence.forEach((moduleId, index) => {
      ...
      item.innerHTML = `
              ...
              <div class="item-controls">
                  <button class="move-up-btn" title="Przesuń w górę" ${
                    index === 0 ? "disabled" : ""
                  }><i class="fas fa-arrow-up"></i></button>
                  <button class="move-down-btn" title="Przesuń w dół" ${
      ...
      item
        ...
        .addEventListener("click", () => moveSequenceItem(index, -1))
      item
  ...
  function generatePrompt() {
    ...
    const parametersList = state.selectedParameters
      .map((param, index) => {
    ...
    const filters = state.selectedParameters
      .map((param) => {
        const paramInfo = PARAMETER_DESCRIPTIONS[param]
        const filterKey = getParameterType(param)

        return `[FILTR${
          filterKey === "standard" ? "" : ` ${filterKey.toUpperCase()}`
        }: ${paramInfo.name}]
...
2. **[Napięcie dialektyczne B]**:
 ...
 - Potencjał integracyjny: [opis]`
        ...
        .replace(
          "[META_ESSENCE]",
          `[Synteza integrująca wglądy z różnych perspektyw]`
        )
        .replace(
          "[LIMITATIONS]",
          `1. **[Ograniczenie A]**: [opis]
...
${state.sequence
  .map((moduleId, index) => {
    const module = MODULE_INFO[moduleId]
    return `${index + 1}. **${moduleId}: ${module.name}** - ${
      module.description
    }`
  })
  .join("\n")}
...
`
  ...
  function validateForm() {
    ...
    if (state.selectedTemplate === "custom" && !state.customTemplate) {
      showAlert(
        "Proszę podać własny szablon lub wybrać jeden z predefiniowanych."
      )
      return false
    }
  ...
  function resetForm() {
    ...
    elements.parameterCheckboxes.forEach(
  ...
  function copyPromptToClipboard() {
    ...
    navigator.clipboard
      ...
      .catch((err) => {
        ...
        showAlert(
          "Nie udało się skopiować do schowka. Proszę spróbować ręcznie zaznaczyć i skopiować tekst."
        )
      })
  }
  ...
  function renderVisualization() {
    ...
    if (state.sequence.length === 0) {
      ...
      text.textContent =
        "Brak modułów w sekwencji. Dodaj moduły, aby wizualizować przepływ procesu."
      text.setAttribute("x", svgWidth / 2 - 200)
    ...
    state.sequence.forEach((moduleId, index) => {
      // Oblicz pozycję węzła (w układzie siatki)
      const row = Math.floor(index / 4)
      const col = index % 4

      const x = col * horizontalSpacing + nodeRadius
      const y = row * verticalSpacing + nodeRadius
    ...
    for (let i = 0; i < nodes.length - 1; i++) {
      ...
      arrow.setAttribute(
        "points",
        `${endX},${endY} ${arrowX + arrowSize * ny},${
          arrowY - arrowSize * nx
        } ${arrowX - arrowSize * ny},${arrowY + arrowSize * nx}`
      )
  ...
  function loadConfiguration() {
    ...
    const configListHtml = savedConfigs
      .map((config, index) => {
        ...
        return `
              <div class="saved-config-item" data-index="${index}">
                  <div class="config-info">
                      ...
                      <p><small>Parametry: ${
                        config.selectedParameters.length
                      }, Moduły: ${config.sequence.length}</small></p>
                  </div>
  ...
  function applyConfiguration(config) {
    ...
    elements.parameterCheckboxes.forEach((checkbox) => {
      checkbox.checked = state.selectedParameters.includes(
        checkbox.dataset.param
      )
    })
    ...
    showModal(
      ...
      `<div class="help-content">
              ...
              <ol>
                  ...
                  <li>Wybierz moduły z lewego panelu, aby zbudować swoją sekwencję destylacyjną</li>
